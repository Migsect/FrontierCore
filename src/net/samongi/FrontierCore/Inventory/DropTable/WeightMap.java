...
public class WeightMap<T>
{
  ...
  {
    if(this.item_weights.containsKey(item)) this.item_weights.put(item, this.item_weights.get(item) + weight);
    this.put(item, weight);
  }
  ...
  public Integer getWeight(T item)
  {
    if(!this.item_weights.containsKey(item)) return 0;
